...
static const struct Module aModule[] = {
  {"fts3", &fts3Module},
  {"fts4", &fts4Module},
  {"fts5", &fts5Module},
  {NULL, NULL}
...
int sqlite3Fts3Init(sqlite3 *db)
{
  ...
  if (db->mallocFailed)
    return SQLITE_NOMEM;
  rc = sqlite3_create_function(db, "fts3_tokenizer", -1, SQLITE_UTF8 | SQLITE_DETERMINISTIC, 0, fts3TokenizerSqlFunc, 0, 0);
  if (rc != SQLITE_OK)
    return rc;
  for (const struct Module *module = aModule; module->name; module++) {
